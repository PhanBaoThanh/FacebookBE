...
from app.Service import ClientService,CommentService,FriendRequestService,FriendService,GroupMemberService,GroupRequestService,GroupService,MessageService,PostConfirmService,PostService,ChatService,ReactionService
import json
...
def getAllPostOfClient(client_id):
    ...
    # return json.dumps(list,indent=4)
    list = main.GetAllPostOfClientByClientId(clientId = client_id)
    return json.dumps(list,indent=4)
...
def getAllPostGroupOfGroup(group_id):
    ...
    #     list.append(PostGroup.serialize())
    # return json.dumps(list,indent=4)
    list = main.GetAllPostOfGroupByGroupId(groupId = group_id)
    return json.dumps(list,indent=4)
...
@app.route('/get-all-post-of-friend-by-client-id/<int:client_id>',methods=['GET'])
def getAllPostOfFriendByClient(client_id):
    list = main.GetAllPostOfFriendByClientId(clientId = client_id)
    return json.dumps(list,indent=4)
...
def getAllCommentByPostId(post_id):
    list = main.GetAllCommentOfPostByPostId(postId = post_id)
    return json.dumps(list,indent=4)

@app.route('/get-all-comment-by-post-id-limit/<int:post_id>',methods=['GET'])
def getAllCommentByPostIdLimit(post_id):
    list = main.GetAllCommentOfPostByPostIdLimit(postId = post_id)
    return json.dumps(list,indent=4)
...
def getAllReactionByPostId(post_id):
    ...
    # return json.dumps(list,indent=4)
    list = main.GetAllReactionOfPostByPostId(postId = post_id)
    return json.dumps(list,indent=4)
...
def getAllFriendByFriendRequest(client_id):
    list = main.GetFriendRequestByClientId(clientId = client_id)
    return json.dumps(list,indent=4)

@app.route('/get-all-friend-by-friend-response/<int:client_id>',methods=['GET'])
def getAllFriendByFriendResponse(client_id):
    list = main.GetFriendResponseByClientId(clientId = client_id)
    return json.dumps(list,indent=4)
...
def getAllGroupByGroupRequest(client_id):
    list = main.GetAllGroupRequestByClientId(clientId = client_id)
    return json.dumps(list,indent=4)
...
def getAllMessageByClientId():
    list = main.GetAllMessageByClientId(client1 = request.args.get('client1'),client2= request.args.get('client2'))
    return json.dumps(list,indent=4)

@app.route('/get-all-message-by-chat-id/<int:chat_id>',methods=['GET'])
def getAllMessageByChatId(chat_id):
    list = main.GetAllMessageByChatId(chatId = chat_id)
    return json.dumps(list,indent=4)
...
@app.route('/message-delete-by-client-id-and-chat-id',methods=['POST'])
def deleteAllMessageByClientId():
    messageJson = request.get_json()
    MessageService.deleteMessageByClientIdAndChatId(chatId = messageJson['chatId'],clientId= messageJson['clientId'])
    return 'Delete Message Success',201
...
@app.route('/post-confirm-no-img',methods=['POST'])
def createPostConfirmNoImg():
    PostConfirmForm = request.get_json()
...
@app.route('/post-confirm-update',methods=['POST'])
def updatePostConfirm():
    PostConfirmForm = request.form
    Img = request.files['image']
    if Img and allowed_file(Img.filename):
        filenameImg = secure_filename(Img.filename)
    ...
    PostConfirm = PostConfirmService.updatePostConfirm(
        postConfirmId = PostConfirmForm['postConfirmId'],
        content = PostConfirmForm['content'],
        img = Img.filename
    )
    if PostConfirm is None:
        return "Update Post Confirm Fail",500
    return "Update Post Confirm Success", 201

@app.route('/post-confirm-update-no-img',methods=['POST'])
def updatePostConfirmNoImg():
    PostConfirmJson = request.get_json()
    PostConfirm = PostConfirmService.updatePostConfirm(
        postConfirmId = PostConfirmJson['postConfirmId'],
        content = PostConfirmJson['content'],
        img = ''
    )
    if PostConfirm is None:
        return "Update Post Confirm Fail",500
...
def getAllPostConfirmByGroupId(group_id):
    ...
    # return json.dumps(list,indent=4)
    list = main.GetAllPostConfirmByGroupId(groupId = group_id)
    return json.dumps(list,indent=4)
...
@app.route('/get-all-friend-request-by-client-id/<int:client_id>',methods=['GET'])
def getAllFriendRequestByClientId(client_id):
    FriendRequests = FriendRequestService.getAllFriendRequestByClientId(clientId = client_id)
    list = []
    for FriendRequest in FriendRequests:
        list.append(FriendRequest.serialize())
    return json.dumps(list,indent=4)
...
@app.route('/friend-request-find-by-sender-id-and-receiver-id',methods=['POST'])
def findFriendRequestBySenderIdAndReceiverId():
    FriendRequestJson = request.get_json()
    return jsonify(FriendRequestService.findFriendRequestBySenderIdAndReceiverId(senderId=FriendRequestJson['senderId'],receiverId=FriendRequestJson['receiverId']).serialize())
...
def getAllMemberFromGroupRequest(group_id):
    list = main.GetAllClientFromGroupRequest(groupId= group_id)
    return json.dumps(list,indent=4)
...
@app.route('/get-all-post-of-client-test/<int:client_id>',methods=['GET'])
def getAllPostOfClientTest(client_id):
    list = main.GetAllPostOfClient(clientId = client_id)
    return json.dumps(list,indent=4)
...
def createChat():
    chatJson = request.get_json()
    ChatRequest = ChatService.CreateChat(client1 = chatJson['client1'],client2 = chatJson['client2'])
    if ChatRequest is None:
        return 'Create Chat Request Fail',500
    return jsonify(ChatRequest.serialize())

@app.route('/get-all-chat-by-client-id/<int:client_id>',methods=['GET'])
def getAllChatByClientId(client_id):
    list = main.GetChatOfClient(clientId = client_id)
    return json.dumps(list,indent=4)

@app.route('/find-chat-by-id',methods=['POST'])
def findChatById():
    chatJson = request.get_json()
    return jsonify(ChatService.FindChatById(id=chatJson['id']).serialize())

@app.route('/delete-chat-by-id/<int:chat_id>',methods=['DELETE'])
def deleteChatById(chat_id):
    ChatService.DeleteChatById(id=chat_id)
    return 'Deleted'

@app.route('/delete-chat-by-client-id',methods=['DELETE'])
def deleteChatByClientId():
    ChatService.DeleteChatByClientId(client1 = request.args.get('client1'),client2=request.args.get('client2'))
    return 'Deleted'