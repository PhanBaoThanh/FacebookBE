...
def GetAllByClientId(clientId):
    return FriendRequest.query.filter(or_(FriendRequest.SenderId==clientId,FriendRequest.ReceiverId==clientId)).order_by(FriendRequest.CreatedAt.desc()).all()

def FindById(id):
    return FriendRequest.query.filter(FriendRequest.FriendRequestId == id).first_or_404()

def FindBySenderIdAndReceiverId(senderId,receiverId):
    return FriendRequest.query.filter(or_(and_(FriendRequest.SenderId==senderId,FriendRequest.ReceiverId==receiverId),and_(FriendRequest.SenderId == receiverId,FriendRequest.ReceiverId == senderId))).first_or_404()
...
def Delete(id):
    friendRequest = FriendRequest.query.filter(FriendRequest.FriendRequestId == id).first()
